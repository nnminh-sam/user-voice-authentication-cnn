...
class CNNModelV2:
    def build(num_classes) -> Sequential:
        model = Sequential([
            Conv1D(16, kernel_size=3, activation=None, padding='same', kernel_regularizer=l2(0.001), input_shape=(16, 1)),
            LeakyReLU(alpha=0.1),
            MaxPooling1D(pool_size=2),

            Conv1D(32, kernel_size=3, activation=None, padding='same', kernel_regularizer=l2(0.001)),
            LeakyReLU(alpha=0.1),
            MaxPooling1D(pool_size=2),
