...
import utils.model_conversion as ModelConversion
...
def split_and_save_sets(features, file_names, train_path, val_path, test_path):
    ...
    X_train, X_temp, y_train, y_temp = train_test_split(
        features, file_names, test_size=0.3, random_state=42
    )
...
def main():
    ...
    USAGE_TEST_PATH: str = "dataset/nnminh_test/audio_1741766932066.bin"
    C_MODEL_NAME: str = "voice_model"
    TFLITE_MODEL_NAME: str = "voice_model"

    features, labels, label_map, reverse_label_map = read_data_and_extract_feature(
        DATASET_PATH
    )
    ...
    with open(C_MODEL_NAME + ".h", "w") as file:
        file.write(ModelConversion.hex_to_c_array(tflite_model, C_MODEL_NAME))
